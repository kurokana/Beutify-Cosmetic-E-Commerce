<x-app-layout>
    <div class="py-12 bg-dark-primary min-h-screen">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="flex flex-col sm:flex-row sm:items-end justify-between mb-8 border-b border-border-subtle pb-6 gap-4">
                <div>
                    <h1 class="text-3xl md:text-2xl font-black tracking-tight leading-tight">
                        <span class="text-gold-light">Riwayat</span>
                        <span class="text-gold">Pesanan</span>
                    </h1>
                    <p class="text-warm-muted text-sm mt-1 font-medium">Lihat semua pesanan Anda</p>
                </div>
            ...
            @if ($orders->isEmpty())
                <div class="bg-dark-secondary rounded-3xl border border-border-subtle shadow-sm p-12 text-center">
                    <svg class="mx-auto w-20 h-20 text-warm-muted mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
                            d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" />
                    </svg>
                    <h2 class="text-xl font-bold text-warm-white mb-2">Belum ada pesanan</h2>
                    <p class="text-warm-muted text-sm mb-6">Anda belum pernah melakukan pembelian. Mulai belanja sekarang!</p>
                    <a href="{{ route('catalog.index') }}"
                        class="inline-flex items-center gap-2 px-6 py-3 bg-gold text-white rounded-xl font-semibold hover:bg-[#d45a92] transition shadow-md">
            ...
            @else
                <div class="space-y-5">
                    @foreach ($orders as $order)
                        <div class="bg-dark-secondary rounded-2xl border border-border-subtle shadow-sm hover:shadow-md transition-all duration-200 overflow-hidden">
                            {{-- Header Order --}}
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 px-6 py-4 border-b border-gold/30/30 bg-gradient-to-r from-white to-[#FFF9FB]">
                                <div class="flex items-center gap-3 flex-wrap">
                                    <span class="font-bold text-warm-white text-sm">{{ $order->order_number }}</span>
                                    @php
                                        $statusConfig = [
                            ...
                            <div class="px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                <div>
                                    <p class="text-xs text-warm-muted mb-1">
                                        {{ $order->items->count() }} produk
                                        &bull; {{ strtoupper($order->courier_name) }} {{ $order->courier_service }}
                                    </p>
                                    <p class="text-xl font-extrabold text-warm-white">
                                ...
                                <div class="flex items-center gap-2 flex-wrap">
                                    @if ($order->status === 'pending_payment')
                                        <a href="{{ route('orders.show', $order->id) }}"
                                            class="inline-flex items-center gap-1.5 px-4 py-2 bg-gold text-white rounded-lg text-sm font-bold hover:bg-[#d45a92] transition shadow-sm">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @endif
                                    <a href="{{ route('orders.show', $order->id) }}"
                                        class="inline-flex items-center gap-1.5 px-4 py-2 border border-gold/30 text-warm-white rounded-lg text-sm font-medium hover:bg-dark-tertiary hover:border-gold transition">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
