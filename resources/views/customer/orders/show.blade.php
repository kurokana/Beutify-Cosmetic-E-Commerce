            ...
            <div class="flex flex-col sm:flex-row sm:items-end justify-between mb-8 border-b border-[#FFD1DC]/40 pb-6 gap-4">
                <div>
                    <h1 class="text-3xl md:text-2xl font-black tracking-tight leading-tight">
                        <span class="text-[#89CFF0]">Detail</span>
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl border border-[#FFD1DC]/40 shadow-sm p-6">
                    ...
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                        <div>
                            <dt class="text-slate-400 font-medium">Nomor Pesanan</dt>
                            <dd class="font-bold text-gray-800 mt-0.5">{{ $order->order_number }}</dd>
                        </div>
                        <div>
                            <dt class="text-slate-400 font-medium">Tanggal Pesanan</dt>
                            <dd class="font-medium text-gray-800 mt-0.5">{{ $order->created_at->translatedFormat('d F Y, H:i') }}</dd>
                        </div>
                        @if ($order->notes)
                            <div class="sm:col-span-2">
                                <dt class="text-slate-400 font-medium">Catatan</dt>
                                <dd class="font-medium text-gray-800 mt-0.5">{{ $order->notes }}</dd>
                            </div>
                        @endif
                    </dl>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-[#FFD1DC]/40 shadow-sm p-6">
                    ...
                    <div class="divide-y divide-[#FFD1DC]/30">
                        @foreach ($order->items as $item)
                            <div class="flex items-start gap-4 py-4 first:pt-0 last:pb-0">
                                <div class="w-14 h-14 rounded-xl bg-gray-50 border border-[#FFD1DC]/40 shrink-0 flex items-center justify-center overflow-hidden">
                                    @php
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    @endif
                                    <p class="text-xs text-slate-400 mt-0.5">{{ $item->quantity }} × Rp {{ number_format($item->price, 0, ',', '.') }}</p>
                                </div>
                                <p class="text-sm font-extrabold text-gray-800 shrink-0">Rp {{ number_format($item->subtotal, 0, ',', '.') }}</p>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-[#FFD1DC]/40 shadow-sm p-6">
                    ...
                    <div class="text-sm">
                        <div class="flex items-center gap-2 mb-1">
                            <span class="font-bold text-gray-800">{{ $order->address->recipient_name }}</span>
                            <span class="text-slate-400">{{ $order->address->phone }}</span>
                            @if ($order->address->label)
                                <span class="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{{ $order->address->label }}</span>
                            @endif
                        </div>
                        <p class="text-slate-600">{{ $order->address->full_address }}, {{ $order->address->district }}, {{ $order->address->city }}, {{ $order->address->province }} {{ $order->address->postal_code }}</p>
                    </div>
                </div>
                @endif
                ...
                <div class="bg-white rounded-2xl border border-[#FFD1DC]/40 shadow-sm p-6">
                    ...
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                        <div>
                            ...
                            <dd class="font-bold text-gray-800 mt-0.5">{{ strtoupper($order->courier_name) }} — {{ $order->courier_service }}</dd>
                        </div>
                        <div>
                            <dt class="text-slate-400 font-medium">Ongkos Kirim</dt>
                            <dd class="font-bold text-gray-800 mt-0.5">Rp {{ number_format($order->shipping_cost, 0, ',', '.') }}</dd>
                        </div>
                        @if ($order->shipping_tracking_number)
                            <div class="sm:col-span-2">
                                <dt class="text-slate-400 font-medium">Nomor Resi</dt>
                                <dd class="font-mono font-bold text-gray-800 mt-0.5">{{ $order->shipping_tracking_number }}</dd>
                            </div>
                        @endif
                    </dl>
                </div>
                ...
                <div class="bg-white rounded-2xl border border-[#FFD1DC]/40 shadow-sm p-6">
                    ...
                    <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-3 text-sm">
                        ...
                        @if ($order->payment->payment_method)
                            <div>
                                <dt class="text-slate-400 font-medium">Metode Pembayaran</dt>
                                <dd class="font-bold text-gray-800 mt-0.5 capitalize">{{ str_replace('_', ' ', $order->payment->payment_method) }}</dd>
                            </div>
                        @endif
                        @if ($order->payment->paid_at)
                            <div>
                                <dt class="text-slate-400 font-medium">Dibayar Pada</dt>
                                <dd class="font-bold text-gray-800 mt-0.5">{{ $order->payment->paid_at->translatedFormat('d F Y, H:i') }}</dd>
                            </div>
                        @endif
                        @if ($order->payment->expired_at && $order->payment->status === 'pending')
                            <div>
                                <dt class="text-slate-400 font-medium">Batas Waktu</dt>
                ...
                <div class="bg-white rounded-2xl border border-[#FFD1DC]/40 shadow-sm p-6">
                    ...
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between text-slate-600">
                            ...
                            <span class="font-bold text-gray-800">Rp {{ number_format($order->subtotal, 0, ',', '.') }}</span>
                        </div>
                        <div class="flex justify-between text-slate-600">
                        ...
                        @if ($order->discount_amount > 0)
                            <div class="flex justify-between text-green-600">
                                <span>Diskon Voucher @if($order->voucher)<span class="text-xs text-slate-400">({{ $order->voucher->code }})</span>@endif</span>
                                <span class="font-bold">- Rp {{ number_format($order->discount_amount, 0, ',', '.') }}</span>
                            </div>
                        @endif
                        <div class="border-t border-[#FFD1DC]/30 pt-3 mt-3 flex justify-between font-black text-gray-800 text-base">
                            <span>Total</span>
                ...
                <div class="flex flex-wrap gap-3">
                    @if ($order->status === 'pending_payment' && $order->payment?->snap_token)
                        <button type="button" onclick="payNow('{{ $order->payment->snap_token }}')"
                            class="inline-flex items-center gap-2 px-6 py-3 bg-[#E86FA3] text-white rounded-xl font-bold hover:bg-[#d45a92] transition shadow-md">
                    ...
                    <a href="{{ route('orders.index') }}"
                        class="inline-flex items-center gap-2 px-6 py-3 border border-[#FFD1DC] text-gray-700 rounded-xl font-medium hover:bg-gray-50 hover:border-[#E86FA3] transition">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
