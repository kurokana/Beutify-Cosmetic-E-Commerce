<x-admin-layout>
    <x-slot name="pageTitle">Edit Merek</x-slot>
    ...
    <div class="space-y-6">
        ...
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-2xl font-bold text-warm-white">Edit Merek</h2>
                <p class="mt-1 text-sm text-warm-gray">Perbarui informasi merek {{ $brand->name }}</p>
            </div>
        </div>
        ...
        <div class="bg-dark-secondary rounded-lg shadow">
            <form action="{{ route('admin.brands.update', $brand) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
                ...
                @if ($brand->logo_path)
                    <div>
                        <label class="block text-sm font-medium text-warm-white mb-2">
                            Logo Saat Ini
                        </label>
                        <div class="flex items-center gap-4">
                            <div class="h-24 w-24 rounded-lg border-2 border-border-subtle overflow-hidden bg-dark-tertiary">
                                <img
                            ...
                            <form
                                action="{{ route('admin.brands.delete-logo', $brand) }}"
                                method="POST"
                                class="inline"
                ...
                <div>
                    <label for="logo" class="block text-sm font-medium text-warm-white mb-2">
                        {{ $brand->logo_path ? 'Ganti Logo' : 'Upload Logo' }}
                    </label>
